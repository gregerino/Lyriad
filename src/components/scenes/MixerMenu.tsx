...
import { MixerIcon } from "@/components/ui/icons";
      ...
      <div className="flex flex-col gap-4">
        <section>
          ...
          <p className="mt-1 text-[11px] text-muted-foreground">
            Ikryssad längd gäller både in och ut när du startar eller pausar allt.
          </p>
          <div className="mt-2 flex flex-col gap-1">
            {fadeDurationsMs.map((ms) => {
              const checked = fadeDurationMs === ms;
              return (
                <label
                  key={ms}
                  className="flex cursor-pointer items-center gap-2 rounded-md px-2 py-1.5 text-xs text-parchment-200 transition hover:bg-ink-700"
                >
                  <input
                    type="checkbox"
                    checked={checked}
                    // Mutually exclusive by hand rather than radios: a radio group
                    // can't be cleared again, and "no fade" has to stay reachable.
                    onChange={() => onFadeDurationChange(checked ? null : ms)}
                    className="focus-ring h-3.5 w-3.5 flex-none accent-ember-400"
                  />
                  {ms / 1000}s
                </label>
              );
